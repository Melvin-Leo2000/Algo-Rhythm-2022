...
export default function Tracker(props) {
  ...
  let stocksymbol = props.name.toUpperCase();
  ...
  useEffect(() => {
    ...
    async function getLatestPrice() {
      try {
        ...
        window.location.replace("/chartnotfound");
        console.log(error);
      }
  ...
  return (
    ...
    <div className='tracker-analytics'> <div className={['price', value].join(' ')}>{pricededuct} ({percentage}%)</div></div>
    </div>
  )
}
